...
export class InjuryTypeController {
  ...
  static getAll(reqInfo: RequestInfo) {
    try {
      ...
      if (items.length === 0) {
        return this.notFound(reqInfo, "لا يوجد أنواع إصابة.");
      }
  ...
  static getById(reqInfo: RequestInfo) {
    try {
      ...
      if (!item) {
        return this.notFound(reqInfo, "Injury type not found.");
      }
      ...
      return this.serverError(reqInfo, "Failed to load injury type.");
    }
  }
